try:
    # Albumentations <= 1.x
    from albumentations import DualIAATransform, to_tuple  # type: ignore
except Exception:  # pragma: no cover
    # Albumentations >= 2.x (IAA transforms were removed, but we only need these
    # classes for training-time augmentations; inference should still import.)
    from albumentations.augmentations.geometric.transforms import to_tuple  # type: ignore
    from albumentations.core.transforms_interface import DualTransform  # type: ignore

    class DualIAATransform(DualTransform):  # type: ignore
...
class IAAAffine2(DualIAATransform):
    ...
    @property
    def processor(self):
        if iaa is None:  # pragma: no cover
            raise RuntimeError("imgaug is not installed (required only for training augmentations).")
        return iaa.Affine(
            self.scale,
