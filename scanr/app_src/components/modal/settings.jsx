...
import { locale, nativeAlert, nativeConfirm, checkUpdate, readStorage, writeToStorage, deleteStorageFile, openFile, getUserFonts } from "../../utils";
import { BUBBLE_CLASSES, PSEUDO_ITALIC_VALUE, normalizeBubbleClassMap } from "../../bubbleClasses";
...
const SettingsModal = React.memo(function SettingsModal() {
  ...
  const [bubbleReadingOrder, setBubbleReadingOrder] = React.useState(context.state.bubbleReadingOrder || "ltr");
  ...
  const fontFamilies = React.useMemo(() => {
    const fams = [];
    (fonts || []).forEach((font) => {
      if (font?.family && !fams.includes(font.family)) fams.push(font.family);
    });
    return fams;
  }, [fonts]);
  ...
  const resolveFont = (family, style) => {
    if (!fonts || !fonts.length || !family) return null;
    if (style) {
      const found = fonts.find((f) => f.family === family && f.style === style);
      if (found) return found;
    }
    return fonts.find((f) => f.family === family) || null;
  };

  const updateBubbleClassEntry = (id, patch) => {
    setBubbleClassMap((prev) => {
      const current = prev && prev[id] ? prev[id] : {};
      return { ...prev, [id]: { ...current, ...patch } };
    });
    setEdited(true);
  };

  const changeBubbleClassFamily = (id, family) => {
    if (!family) {
      updateBubbleClassEntry(id, {
        fontFamily: "",
        fontStyleName: "",
        fontPostScriptName: "",
        fontName: "",
    ...
    const font = resolveFont(family, null);
    updateBubbleClassEntry(id, {
      fontFamily: family,
      fontStyleName: font?.style || "",
      fontPostScriptName: font?.postScriptName || family,
      fontName: font?.name || family,
      syntheticItalic: false,
    });
  };

  const changeBubbleClassStyle = (id, family, style) => {
    if (!family) return;
    if (style === PSEUDO_ITALIC_VALUE) {
      const current = bubbleClassMap?.[id] || {};
      const fallbackStyle = current.fontStyleName || "Regular";
      updateBubbleClassEntry(id, {
        syntheticItalic: true,
        fontStyleName: fallbackStyle,
      });
      return;
    }
    const font = resolveFont(family, style);
    updateBubbleClassEntry(id, {
      fontStyleName: style,
      fontPostScriptName: font?.postScriptName || family,
      fontName: font?.name || family,
  ...
  const save = (e) => {
    ...
    if (bubbleReadingOrder !== context.state.bubbleReadingOrder) {
      context.dispatch({
        type: "setBubbleReadingOrder",
        readingOrder: bubbleReadingOrder,
      });
    }
    ...
    if (JSON.stringify(bubbleClassMap) !== JSON.stringify(context.state.bubbleClassMap)) {
      context.dispatch({
            ...
            <div className="settings-group">
              ...
              <div className="field">
	            ...
	            <div className="settings-group">
                  ...
                  if (family && !families.includes(family)) families.unshift(family);
                  const familyFonts = (fonts || []).filter((font) => font.family === family);
                  const styleOptions = familyFonts.map((font) => font.style);
                  if (entry.fontStyleName && !styleOptions.includes(entry.fontStyleName)) {
                    styleOptions.unshift(entry.fontStyleName);
                  }
                  const styleValue = entry.syntheticItalic ? PSEUDO_ITALIC_VALUE : entry.fontStyleName || "";
                  return (
                    <div className="bubble-class-row" key={cls.id}>
                      <div className="bubble-class-name">{locale[cls.labelKey] || cls.labelFallback || cls.id}</div>
                      <div className="bubble-class-controls">
                        <select
                          value={family || ""}
                          onChange={(e) => changeBubbleClassFamily(cls.id, e.target.value)}
                          className="topcoat-textarea"
                          disabled={!families.length && !family}
                        >
                          <option value="">{locale.settingsBubbleFontDefault || "Default"}</option>
                          {!families.length && (
                            <option value="">{locale.settingsBubbleFontNoFonts || "No fonts"}</option>
                          )}
                          {families.map((fam) => (
                        ...
                        <select
                          value={styleValue}
                          onChange={(e) => changeBubbleClassStyle(cls.id, family, e.target.value)}
                          className="topcoat-textarea"
                          disabled={!family}
                        >
                          {!styleOptions.length && (
                            <option value="">{locale.settingsBubbleFontNoStyles || "Style"}</option>
                          )}
                          ...
                          <option value={PSEUDO_ITALIC_VALUE}>
                            {locale.settingsBubbleFontPseudoItalic || "Pseudo italic"}
                          </option>
                        </select>
                      </div>
                    </div>
              ...
              <div className="field-descr">{locale.settingsBubbleFontHint || "Applied on step 4 when bubble classification is confident."}</div>
            </div>
            <div className="settings-group">
