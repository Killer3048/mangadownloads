...
import { locale, nativeAlert, setActiveLayerText, getCurrentSelection, getSelectionBoundsHash, startSelectionMonitoring, stopSelectionMonitoring, getSelectionChanged, createTextLayerInSelection, createTextLayersInStoredSelections, alignTextLayerToSelection, alignTextLayerToBubbleAI, aiLoadBubbleModel, aiUnloadBubbleModel, changeActiveLayerTextSize, getStyleObject, scrollToLine } from "../../utils";
...
const PreviewBlock = React.memo(function PreviewBlock() {
  ...
  const selectionCheckInFlight = React.useRef(false);
  ...
  const checkForSelectionChange = React.useCallback(() => {
    ...
    if (selectionCheckInFlight.current) return;
    selectionCheckInFlight.current = true;
    
    getSelectionChanged((selection) => {
      selectionCheckInFlight.current = false;
      if (selection) {
        ...
        const storedHashes = new Set((context.state.storedSelections || []).map((s) => getSelectionBoundsHash(s)));
        
        // Si la sélection n'est pas déjà stockée, l'ajouter
        if (!storedHashes.has(newHash)) {
  ...
  React.useEffect(() => {
    if (context.state.multiBubbleMode) {
      ...
      // Vérifier les changements toutes les 250ms (avec protection contre les appels simultanés)
      selectionCheckInterval.current = setInterval(checkForSelectionChange, 250);
    } else {
  ...
  const toggleAI = async () => {
    ...
    setAiBusy(true);
    try {
      if (!context.state.useAI) {
        await aiLoadBubbleModel();
        context.dispatch({ type: "setUseAI", value: true });
      } else {
        await aiUnloadBubbleModel();
        context.dispatch({ type: "setUseAI", value: false });
      }
    } catch (e) {
      nativeAlert(
        (locale.errorAiServerNotRunning || "AI server not reachable. Start it with: python bubble_detector/server.py") +
          (e && e.message ? "\n\n" + e.message : ""),
        locale.errorTitle,
        true
      );
      ...
      <div className="preview-top">
        ...
        <div className="preview-top_main-controls">
          ...
          <button className="preview-top_big-btn preview-top_big-btn--small topcoat-button--large" title={locale.alignLayerDescr} onClick={() => {
            const padding = context.state.internalPadding || 0;
            if (context.state.useAI) {
              alignTextLayerToBubbleAI(context.state.resizeTextBoxOnCenter, padding).catch((e) => console.error(e));
            } else {
          ...
          <button
            className={`preview-top_ai-btn topcoat-button--large ${context.state.useAI ? "preview-top_ai-btn--active" : ""}`}
            title={context.state.useAI ? (locale.aiDisable || "Disable AI bubble detection") : (locale.aiEnable || "Enable AI bubble detection")}
            onClick={toggleAI}
            disabled={aiBusy}
          >
            {aiBusy ? (locale.aiLoading || "AI...") : (context.state.useAI ? (locale.aiOn || "AI ON") : (locale.aiUse || "USE AI"))}
          </button>
